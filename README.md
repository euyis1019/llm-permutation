...
5. RandOpt/零阶优化 一类方法直接对权重加入参数扰动，再依据前向任务分数搜索或筛选候选。本仓库只做了这类高斯权重扰动的响应标定，没有执行候选优化。这里的“小扰动”具体指：在最低预注册档，噪声标准差为百万分之一，约 1.31% 的 BF16 权重存储值实际改变，而整个权重向量的相对变化不到十万分之一；即使如此，最终 logits 漂移仍与全层随机 permutation 落在相近尺度。
