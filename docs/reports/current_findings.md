...
FFN 联动置换在数学上严格等价；实际出现的差异来自 GPU 浮点矩阵乘的归约顺序。当前 vLLM 栈中，只在对齐的 8-neuron 小块内部重排可以做到逐比特不变；一旦跨出这个边界，就失去零漂移保证，实质性跨块通常很快进入同一数量级的数值漂移。漂移尺度与常见的 permutation 几何距离没有单调关系，单次跨边界的相邻交换也可能达到全局随机置换的 logits 漂移尺度。RandOpt 式权重扰动与 permutation 的机制不同，却能在某些小扰动档产生相近的 logits 响应。已有 20 组随机置换的 benchmark 结果显示：Instruct 模型整体略降，Base 模型整体略升；换半数据验证又显示，从候选池里挑出的额外优势基本不能迁移。候选池相对原始 baseline 的方向仍然存在，但其机制和跨模型可泛化性尚未解决。
...
最直观的例子是奇数起点的相邻交换。按零基索引，它包含 `(7,8)`、`(15,16)` 这样的边界交换：几何位移只有一个位置，却把乘积项送进相邻归约块。完整 Base 模型中，这一条件的最后 token logits 漂移中位数为 0.009489，而全局随机置换为 0.009879，已经处于同一数量级。相反，位移同样很小但保持在对齐块内的偶数起点相邻交换可以逐比特不变。
...
需要把“方向差异”和“方差差异”分开。二十组置换种子之间，Instruct 与 Base 的六任务标准差分别约为 0.19 和 0.21 个百分点，不能称为显著不同；真正清楚的是二十组全部同号且方向相反。稳定性差异则来自同权重重复基线：Base 六项均为零波动，Instruct 只有 GSM8K 留有残余波动。
...
## 7. RandOpt 式权重扰动与 permutation 的共同尺度

RandOpt 一类方法直接对权重加入参数扰动，再用前向任务分数搜索或筛选候选，与零阶参数优化相邻。它和 permutation 的机制不同：正确 permutation 在精确算术下不改变函数，而高斯参数扰动会真实改变权重和模型函数。

最终轮没有运行完整 RandOpt，也没有证明某个扰动候选变成了更好的权重；它做的是响应标定。多档全模型 BF16 高斯扰动中，一些很小的扰动已经让最终 logits 漂移落到全层随机 permutation 的参考尺度。可以据此说，直接参数扰动经过模型的中间计算传播后，可能在 logits 层产生与 permutation 同量级的响应。由于该实验没有逐层捕获高斯臂的 activation，不能进一步声称已经定位了具体中间层的放大路径。
...
2. 几何位移大小不是可靠的漂移旋钮；是否跨越目标 kernel 的归约边界更关键。
3. 这种变化不是必然退化：在当前测试中，Instruct 整体负向，Base 整体正向。
4. 小幅直接权重扰动与 permutation 可以在 logits 层出现相近响应尺度，但机制并不相同。
5. 方向并非跨模型统一，因此不存在“permutation 天生有利”或“天生有害”的简单规律。
6. 如果目标是严格保持原模型行为，当前最可靠方案仍是经过目标部署 kernel 验证的块内免费置换，而不是寄希望于 benchmark 平均值恰好提升。
...
4. **不能把高斯响应标定写成 RandOpt 已经优化成功。** 本仓库没有执行候选搜索，也没有证明更好的权重已经产生。
5. **不能把 Instruct/Base 的方向差异外推到其他模型、kernel、GPU 或采样配置。**
