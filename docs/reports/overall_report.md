...
在当前 RTX 4090、PyTorch 和 vLLM 组合上，我们找到了一类逐比特不变的置换：通道只能在各自的对齐 8 通道块内重排。实质性跨块后，输出通常进入一个由 kernel 决定的非零误差带。漂移大小并不随常见的几何位移连续增长：在奇数起点的相邻交换中，每个被换通道只移动一个位置，但其中包含跨块配对，完整模型的 logits 漂移仍可达到全局随机置换的同一数量级。对 Base 模型做全 36 层随机跨块置换时，32 条 prompt 的最后一个 token logits 的 `rel_l2` 中位数是 0.009879。这里的 0.009879 是整条 logits 向量的相对 L2 距离，不能读成每个 logit 平均变化约 1%。

Benchmark 上的变化较小，主要出现在需要较长生成的任务。20 个全层随机置换中，Instruct 的六任务等权平均相对一个 baseline 为 `-0.79 ± 0.19` 个百分点，Base 为 `+0.49 ± 0.21` 个百分点；Instruct 二十组全负，Base 二十组全正。这里最清晰的是方向分离，不是置换种子间方差分离，因为两组标准差很接近。运行稳定性的明显差异出现在同权重重复基线：Base 六项 correctness 完全一致，Instruct 的 GSM8K 仍有残余波动。现有实验只说明原始布局位于两个置换分布的不同位置，还没有解释这种位置差异来自偶然、kernel 布局偏好、post-training 过程，还是其他机制。

RandOpt 一类方法会直接对权重加入参数扰动，再依据前向任务分数搜索或筛选候选，与零阶参数优化相邻。为标定这种扰动，本仓库对全模型 BF16 权重加入了多档高斯噪声。实验发现，即使只有很小一部分 BF16 权重实际改变，扰动经过模型中间计算传播后，最终 logits 也能达到与全层 permutation 相近的 `rel_l2`。本实验没有捕获这条高斯臂的逐层 activation，也没有执行 RandOpt 的候选选择或优化，因此它证明的是 logits 响应尺度相近，不是已经找到了更好的权重。它也不是一个已经证明的通用测量下限，因为高斯扰动确实改变了参数和有限精度模型函数。
